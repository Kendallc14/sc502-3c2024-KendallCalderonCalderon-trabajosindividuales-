document.addEventListener('DOMContentLoaded', function () {
    ...
    //Ya hay comunicacion del front-end al back-end pero necesito que se despliegue la lista de comentarios en cada una de las tareaa
    
    async function loadComments(taskId) {
        try {
            const response = await fetch(API_URL, {
                method: 'GET',
                credentials: 'include', // Si usas sesiones en el backend
            });
    
            if (response.ok) {
                const comments = await response.json();
                return comments; // Devuelve los comentarios en formato JSON
            } else {
        ...
        tasks.forEach(function (task) {
            ...
            if (task.comments && task.comments.length > 0) {
                ...
                task.comments.forEach(comment => {
                    commentsList += `<li class="list-group-item">${commentsList}
    ...
    document.querySelectorAll('.remove-comment').forEach(function (button) {
        button.addEventListener('click', async function (e) {
            ...
            const response = await fetch(`backend/comments.php?id=${commentId}&task_id=${taskId}`, {
                method: 'DELETE',
                credentials: 'include'
            });
